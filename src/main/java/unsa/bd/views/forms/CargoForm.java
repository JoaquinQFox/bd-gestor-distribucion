...
public class CargoForm extends JInternalFrame {
    // Colores
    ...
    private JPanel buildHeader() {
        ...
        JLabel titleLabel = new JLabel("CARGO");
    ...
    private static JTable getJTable(DefaultTableModel model) {
        JTable tableCargo = new JTable(model) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
            }
        };
        tableCargo.setFont(TABLE_FONT);
        tableCargo.setRowHeight(28);
        tableCargo.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        tableCargo.setGridColor(BORDER);
        tableCargo.setSelectionBackground(TABLE_SELECTION);
        tableCargo.setForeground(Color.BLACK);
        tableCargo.setFillsViewportHeight(true);
        return tableCargo;
    }
    ...
    private void generateButtonsAction() {
        ...
        updateButton.addActionListener(e -> {
            ...
            Cargo cargo = new Cargo(cod, nom, estReg);
            try {
                CargoDAO dao = new CargoDAO();
                switch (modo) {
                    case "ADD" -> {
