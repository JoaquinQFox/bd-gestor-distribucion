...
public class CiudadForm extends JInternalFrame {
    // Colores
    ...
    private JPanel buildHeader() {
        ...
        JLabel titleLabel = new JLabel("CIUDAD");
    ...
    private JPanel getRegisterSectionPanel() {
        ...
        registerFormPanel.add(makeStyledLabel("Región:"), c);

        regComboBox = makeStyledComboBox();
        setJComboBoxEditable(regComboBox, false);
        c.gridx = 1; c.gridy = 2; c.weightx = 1.0; c.fill = GridBagConstraints.HORIZONTAL;
        registerFormPanel.add(regComboBox, c);
    ...
    private JComboBox<Region> makeStyledComboBox() {
        JComboBox<Region> box = new JComboBox<>();
        loadRegionesInComboBox(box);
        ...
        box.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(
                ...
                if (value instanceof Region r) {
                    label.setText(r.getRegNom());
                }
                return label;
            }
        });
    ...
    private void setJComboBoxEditable(JComboBox<Region> box, boolean editable) {
    ...
    private void loadRegionesInComboBox(JComboBox<Region> box) {
        ...
        try {
            List<Region> lista = dao.listarTodo();
            for (Region r : lista) {
                box.addItem(r);
            }
        } catch (Exception e) {
            System.err.println("Error al cargar regiones en combobox ciudad");
        }
    }
    ...
    private static JTable getJTable(DefaultTableModel model) {
        JTable tableCiu = new JTable(model) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
            }
        };
        tableCiu.setFont(TABLE_FONT);
        tableCiu.setRowHeight(28);
        tableCiu.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        tableCiu.setGridColor(BORDER);
        tableCiu.setSelectionBackground(TABLE_SELECTION);
        tableCiu.setForeground(Color.BLACK);
        tableCiu.setFillsViewportHeight(true);
        return tableCiu;
    }
    ...
    private static void getTableData(DefaultTableModel model) {
        ...
        try {
            ...
            for (Ciudad c : ciudades) {
                String nombreRegion = "Cod: " + c.getCiuRegCod();
                for (Region r : regiones) {
                    if (r.getRegCod() == c.getCiuRegCod()) {
                        nombreRegion = r.getRegNom();
                        break;
                    }
                }
    ...
    private void generateButtonsAction() {
        addButton.addActionListener(e -> {
            if (carFlaAct == 0) {
                ...
                setJComboBoxEditable(regComboBox, true);
        ...
        modifyButton.addActionListener(e -> {
            if (carFlaAct == 0) {
                ...
                String labelReg = table.getValueAt(row, 2).toString();
                for (int i = 0; i < regComboBox.getItemCount(); i++) {
                    if (regComboBox.getItemAt(i).getRegNom().equals(labelReg)) {
                        regComboBox.setSelectedIndex(i);
                        break;
                    }
                }

                estRegField.setText(table.getValueAt(row, 3).toString());
                setFieldEditable(nomField, true);
                setJComboBoxEditable(regComboBox, true);
                carFlaAct = 1;
                modo = "MODIFY";
        ...
        updateButton.addActionListener(e -> {
            ...
            Ciudad ciudad = new Ciudad(cod, nom, regCod, estReg);
            try {
                CiudadDAO dao = new CiudadDAO();
                switch (modo) {
                    case "ADD" -> {
                    ...
                    case "INACTIVATE" -> {
                        dao.inactivar(ciudad.getCiuCod());
                    ...
                    case "REACTIVATE" -> {
                        dao.reactivar(ciudad.getCiuCod());
