...
import unsa.bd.dao.TipoFacturaDAO;
import unsa.bd.model.TipoFactura;
import unsa.bd.views.forms.util.FormMode;
import javax.swing.*;
import java.util.List;

public class TipoFacturaForm extends BaseForm {
    ...
    protected void loadTableData() {
        TipoFacturaDAO dao = new TipoFacturaDAO();
        try {
            List<TipoFactura> tipos = dao.listarTodo();
    ...
    protected void onAdd() throws Exception {
        TipoFactura t = buildFromFields();
        if (t.getTipFacCod().isEmpty()) throw new Exception("Llenar el campo de código");
        if (!t.getTipFacCod().matches("\\d+")) throw new Exception("El campo código solo acepta números");
        new TipoFacturaDAO().agregar(buildFromFields());
    }

    @Override
    protected void onModify() throws Exception {
        new TipoFacturaDAO().modificar(buildFromFields());
    }

    @Override
    protected void onDelete() throws Exception {
        new TipoFacturaDAO().eliminar(buildFromFields().getTipFacCod());
    }

    @Override
    protected void onInactivate() throws Exception {
        new TipoFacturaDAO().inactivar(buildFromFields().getTipFacCod());
    }

    @Override
    protected void onReactivate() throws Exception {
        new TipoFacturaDAO().reactivar(buildFromFields().getTipFacCod());
    }
